  ...
  collectStatusIssues: (
    ...
    for (const account of accounts) {
      ...
      if (ext.configured && !ext.enabled) {
        issues.push({
          level: "warn",
          message: `OpenClaw Kakao "${ext.accountId}" is configured but disabled`,
        });
      }
      ...
      if (ext.running && ext.lastInboundAt) {
        ...
        if (silentMs > 30 * 60 * 1000) {
          issues.push({
            level: "warn",
            message: `OpenClaw Kakao "${ext.accountId}" has not received messages for ${Math.round(silentMs / 60000)} minutes`,
          });
        }
      }
    }
