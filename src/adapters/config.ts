 ...
 * Uses OpenClaw standard accounts structure: channels.openclaw-kakao.accounts.<accountId>
 */
...
/**
 * Get the openclaw-kakao config object
 */
function getKakaoTalkchannelConfig(cfg: unknown): ConfigObject | undefined {
  ...
  const talkchannelConfig = (channels as ConfigObject)["openclaw-kakao"];
  if (!talkchannelConfig || typeof talkchannelConfig !== "object") return undefined;
  return talkchannelConfig as ConfigObject;
}

/**
 * Get accounts object from openclaw-kakao config
...
/**
 * Resolve OpenClaw Kakao account from configuration
...
function resolveKakaoTalkChannel(cfg: unknown, accountId: string): ResolvedKakaoTalkChannel {
  ...
  if (!validationResult.success) {
    const errors = validationResult.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid OpenClaw Kakao configuration: ${errors}`);
  }
