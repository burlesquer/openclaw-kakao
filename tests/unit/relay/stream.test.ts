...
describe("Relay Stream", () => {
  ...
  describe("startRelayStream", () => {
    ...
    it("should use configured relayUrl", async () => {
      const { startRelayStream } = await import("../../../src/relay/stream.js");
      ...
      // Verify createSession was called with configured URL
      expect(createSession).toHaveBeenCalledWith("https://relay.example.com/");
    });
  ...
  describe("onConnected / onDisconnected callbacks", () => {
    it("should forward onConnected to connectSSE handlers", async () => {
      ...
      const mockAccount: ResolvedKakaoTalkChannel = {
        talkchannelId: "test",
        enabled: true,
        config: { enabled: true, relayUrl: "https://relay.example.com/", sessionToken: "tok", dmPolicy: "open" },
      };
